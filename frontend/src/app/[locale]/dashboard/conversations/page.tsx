...
export default function ConversationsPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] overflow-hidden">
      ...
      <div className="w-[300px] border-r border-white/5 flex flex-col shrink-0" style={{ background: 'var(--bg-secondary)' }}>
        <div className="p-4 border-b border-white/5">
          <div className="relative mb-4">
            ...
            <input 
              type="text" 
              placeholder="Search..." 
              className="w-full bg-white/5 border border-white/10 rounded-xl py-2 pl-10 pr-4 text-sm text-white focus:outline-none focus:border-primary/50 transition-all"
              value={searchQuery}
              ...
              >
                ...
                <div className="relative">
                  ...
                  <div className={`absolute -right-0.5 -bottom-0.5 w-3.5 h-3.5 rounded-full border-2 border-slate-900 flex items-center justify-center ${conv.handling_mode === 'ai' ? 'bg-violet-500' : 'bg-blue-500'}`}>
                    {conv.handling_mode === 'ai' ? <Bot className="w-2 h-2 text-white" /> : <User className="w-2 h-2 text-white" />}
                  </div>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2 overflow-hidden">
                    <p className="text-sm font-bold text-white truncate">{conv.customer_name || `PSID: ${conv.customer_psid}`}</p>
                    <span className="text-[10px] text-dark-500 whitespace-nowrap">
                      {conv.last_message_at ? new Date(conv.last_message_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}
                    </span>
                  </div>
                  <div className="flex items-center justify-between mt-0.5">
                    <p className={`text-xs truncate ${conv.unread_count > 0 ? 'text-white font-medium' : 'text-dark-500'}`}>
      ...
      <div className="flex-1 flex flex-col relative" style={{ background: 'var(--bg-primary)' }}>
        {selected ? (
          <>
            {/* Header */}
            <div className="h-16 px-6 border-b border-white/5 flex items-center justify-between backdrop-blur-xl bg-black/20 z-10">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-primary/20 flex items-center justify-center text-primary text-xs font-bold">
                  {(selected.customer_name || 'C').charAt(0)}
                </div>
                <div>
                   <h2 className="text-sm font-bold text-white leading-none">{selected.customer_name}</h2>
              ...
              <div className="flex items-center gap-4">
                 <div className="flex items-center bg-white/5 p-1 rounded-xl border border-white/5">
                    <button 
                      ...
                      className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${selected.handling_mode === 'ai' ? 'bg-violet-600 text-white shadow-lg' : 'text-dark-500 hover:text-dark-300'}`}
                    >
                      <Bot className="w-3.5 h-3.5" /> AI Mode
                    </button>
                    <button 
                      onClick={() => selected.handling_mode !== 'human' && toggleMode()}
                      className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${selected.handling_mode === 'human' ? 'bg-blue-600 text-white shadow-lg' : 'text-dark-500 hover:text-dark-300'}`}
                    >
                      <User className="w-3.5 h-3.5" /> Human Mode
                    </button>
                 </div>
                 <button className="p-2 h-10 w-10 text-dark-500 hover:text-white transition-colors">
                 ...
                 if (msg.sender_type === 'system') {
                   return (
                     <div key={msg.id} className="flex justify-center">
                        <span className="px-3 py-1 rounded-full bg-white/5 text-[10px] font-bold text-dark-500 uppercase tracking-widest border border-white/5">
                          {msg.content_text}
                        </span>
                     </div>
                   );
                      ...
                      <div className={`max-w-[70%] group`}>
                         <div className={`px-4 py-3 rounded-2xl text-sm leading-relaxed relative ${
                           msg.direction === 'outbound' 
                             ...
                             : 'bg-white/5 text-dark-200 border border-white/10 rounded-bl-sm'
                         }`}>
            ...
            {/* Input Area */}
            <div className="p-4 border-t border-white/5 backdrop-blur-xl bg-black/20">
               {selected.handling_mode === 'ai' ? (
                 <div className="bg-violet-500/10 border border-violet-500/20 p-4 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                       ...
                       <div>
                          <p className="text-sm font-bold text-white leading-none">Xeni AI is Handling</p>
                 ...
                 <div className="flex items-end gap-3">
                   <div className="flex-1 bg-white/5 border border-white/10 rounded-2xl p-2 focus-within:border-primary/50 transition-all flex flex-col">
                      <textarea 
                        ...
                        rows={1}
                        className="w-full bg-transparent border-0 ring-0 focus:ring-0 p-2 text-sm text-white resize-none"
                        value={reply}
                      ...
                      />
                      <div className="flex items-center justify-between px-2 pt-2 pb-1 border-t border-white/5 mt-2">
                         <div className="flex items-center gap-2">
                            <button className="p-1.5 text-dark-500 hover:text-white transition-colors"><ShoppingBag className="w-4 h-4" /></button>
                            <button className="p-1.5 text-dark-500 hover:text-white transition-colors"><Bot className="w-4 h-4" /></button>
                         </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-10 opacity-40">
             ...
             <h2 className="text-xl font-heading font-bold text-white mb-2">No Conversation Selected</h2>
      ...
      <div className={`w-[260px] border-l border-white/5 flex flex-col shrink-0 transition-all ${selected ? 'translate-x-0' : 'translate-x-full'}`} style={{ background: 'var(--bg-secondary)' }}>
        {selected && (
          <div className="flex flex-col h-full overflow-y-auto custom-scrollbar">
            <div className="p-6 text-center border-b border-white/5">
               ...
               <h3 className="font-heading font-bold text-white mb-1">{selected.customer_name}</h3>
               ...
               <div>
                  ...
                  <div className="space-y-3">
                     <div className="flex items-center justify-between">
                        ...
                        <code className="text-[11px] text-white bg-white/5 px-2 py-0.5 rounded">{selected.customer_psid}</code>
                     </div>
                     <div className="flex items-center justify-between">
                        <span className="text-xs text-dark-400">Total Orders</span>
                        <span className="text-xs font-bold text-white">{customerOrders.length}</span>
                     </div>
                  </div>
               </div>
                    ...
                    {customerOrders.length === 0 ? (
                      ...
                      customerOrders.slice(0, 3).map(order => (
                        <div key={order.id} className="p-3 bg-white/5 border border-white/5 rounded-xl flex items-center justify-between">
                           <div>
                              <p className="text-xs font-bold text-white">#XENI-{order.id.slice(0,4)}</p>
